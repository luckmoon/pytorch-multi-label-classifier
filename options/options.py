...
class Options():
    def __init__(self):
        ...
        self.parser.add_argument('--name', required=True, default='test',
                                 help='subdirectory name for training or testing, snapshot, splited dataset and test results exist here')
        self.parser.add_argument('--mode', required=True, default='Train',
                                 help='run mode of training or testing. [Train | Test | train | test]')
        self.parser.add_argument('--model', required=True, default='alexnet',
                                 help='model type. [Alexnet | LightenB | VGG16 | Resnet18 | ...]')
        ...
        self.parser.add_argument('--batch_size', type=int, default=1,
                                 help='batch size of network input. Note that batch_size should only set to 1 in Test mode')
        ...
        self.parser.add_argument('--crop', type=str, default='CenterCrop',
                                 help='crop type, candidates are [NoCrop | RandomCrop | CenterCrop | FiveCrop | TenCrop]')
        ...
        self.parser.add_argument('--padding', action='store_true',
                                 help='default false. If true, image will be padded if scaled box is out of image boundary')
        ...
        self.parser.add_argument('--loss_weight', type=str, default='',
                                 help='list. Loss weight for cross entropy loss.For example set $loss_weight to [1, 0.8, 0.8] for a 3 labels classification')

        ## for test
        self.parser.add_argument('--top_k', type=str, default='(1,)',
                                 help='tuple. We only take top k classification results into accuracy consideration')
        self.parser.add_argument('--score_thres', type=str, default='0.0',
                                 help='float or list. We only take classification results whose score is bigger than score_thres into recall consideration')
        ...
        self.parser.add_argument('--display_port', type=int, default=8097,
                                 help='port of visdom server for web display. Result will show on `localhost:$display_port`')
        self.parser.add_argument('--image_ncols', type=int, default=0,
                                 help='if positive, display all images in a single visdom web panel with certain number of images per row.')
        self.parser.add_argument('--html', action='store_false',
                                 help='defalt true. Do not save intermediate training results to [opt.dir]/[opt.name]/web/')
        self.parser.add_argument('--update_html_freq', type=int, default=10, help='frequency of saving training results to html')
        self.parser.add_argument('--display_train_freq', type=int, default=10,
                                 help='print train loss and accuracy every $train_freq batches iteration')
        self.parser.add_argument('--display_validate_freq', type=int, default=10,
                                 help='test validate dateset every $validate_freq batches iteration')
